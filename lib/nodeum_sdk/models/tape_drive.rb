...
module Nodeum
  class TapeDrive < TapeDriveDevice
    ...
    attr_accessor :use_to

    attr_accessor :use_by

    attr_accessor :barcode

    attr_accessor :task_id

    attr_accessor :use_file_processed_size

    attr_accessor :use_file_size_to_process

    attr_accessor :use_file_name_processed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Nodeum::TapeDrive` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Nodeum::TapeDrive`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
